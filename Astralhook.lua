...
local cfglocation = " Astralhook.dev.cfg/"      
makefolder(" Astralhook.dev.cfg")      
...
function library:New(name)      
	local menu = {} 
        ...
        local Astralhook = INST("ScreenGui") 
        ...
        Astralhook.Name = "electric boogalo" 
        Astralhook.ResetOnSpawn = false 
        Astralhook.ZIndexBehavior = "Global" 
        Astralhook.DisplayOrder = 420133769 
    
        local UIScale = INST("UIScale") 
        UIScale.Parent = Astralhook 
        ...
        but.Parent = Astralhook 
    
        local cursor = INST("ImageLabel") 
        cursor.Name = "cursor" 
        cursor.Parent = Astralhook 
        ...
        game:GetService("RunService").RenderStepped:connect(function() 
            cursor.Visible = Astralhook
.Enabled 
            cursor.Position = UDIM2(0,Mouse.X-32,0,Mouse.Y-28) 
        end) 
    
        Menu.Name = "Menu" 
        Menu.Parent = Astralhook 
        ...
        game:GetService("UserInputService").InputBegan:Connect(function(key) 
            if key.KeyCode == Enum.KeyCode.Insert then 
                Astralhook
    .Enabled = not Astralhook
    .Enabled 
                library.uiopen = Astralhook
    .Enabled 
            end 
        end) 
...
TextLabel.Parent = KeybindList.TextLabel.Frame
    ...
    UIListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Left
        ...
        Astralhook.Parent = game.CoreGui 
...
makefolder(" Astralhook.dev.lua")      

local allluas = {}  

for _,lua in pairs(listfiles(" Astralhook.dev.lua")) do  
	local luaname = GSUB(lua, " Astralhook.dev.lua\\", "")  
	INSERT(allluas, luaname)  
end  

RunService.RenderStepped:Wait()      

local gui = library:New(" Astralhook.dev")      
local gui = library:New("0.0.1")
...
luascripts:Element("Button", "load", {}, function()  
	loadstring(readfile(" Astralhook.dev.lua\\"..values.luas["lua scripts"].lua.Scroll))()  
end)  
...
antiaim:Element("Dropdown", "pitch", {options = {"none", "up", "down", "dogshit", "180", "funny", " Astralhook.dev", "Lucid2", "Ludic3", "random", "God", "fucker", "cool", "cool2", "back"}})  
...
client:Element("Toggle", "infinite cash", nil, function(tbl)      
	if tbl.Toggle then      
		LocalPlayer.Cash.Value = 9999999999999 inf
	end      
end)      
		...
		client:Element("Toggle", "buy any grenade")
				client:Element("Toggle", "rape watermark", {}, function(tbl)   
					if tbl.Toggle then
						...
						game.CoreGui" Astralhook.dev:Destroy()
					end
					end)
...
local" Astralhook.dev = Instance.new("ScreenGui")
local Main = Instance.new("TextButton")
local Frame = Instance.new("Frame")
    
" Astralhook.dev.Name = " Astralhook.dev"" Astralhook.dev.Parent = game.CoreGui" Astralhook.dev.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

    
Main.Size = UDim2.new(0,200,0,40)
Main.Transparency = 1
Main.Parent =" Astralhook.dev
...
CreateHitElement("Loading" Astralhook.dev,xyz..." Astralhook.dev loaded Enjoy",Color3.new(1,1,0))
...
" Astralhook.dev.Enabled = TeamDamage
	spawn(function()
		while TeamDamage do
			pcall(function()
				local UIListLayout = Instance.new("UIListLayout")
...
RunService.RenderStepped:Connect(function(step)     
	...
	if PlayerIsAlive then      
      ...
      if values.rage.angles["body roll"].Dropdown == "-262" then  
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "none" and 0 or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "dogshit" and -5/93 -13 or values.rage.angles["pitch"].Dropdown == "funny" and 20 or values.rage.angles["pitch"].Dropdown == " Astralhook.dev" and math.huge -5/0 -1 or values.rage.angles["pitch"].Dropdown == "Lucid2" and 23 or values.rage.angles["pitch"].Dropdown == "cool2" and -13 or values.rage.angles["pitch"].Dropdown == "cool" and 12 or values.rage.angles["pitch"].Dropdown == "back" and -92 or values.rage.angles["pitch"].Dropdown == "God" and 15 or values.rage.angles["pitch"].Dropdown == "fucker" and -13 or values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-100, 100)/10 or 2.5  
...
LocalPlayer.Cash:GetPropertyChangedSignal("Value"):Connect(function(cash)      
	if values.misc.client["infinite cash"].Toggle and cash ~= 16000 then      
		LocalPlayer.Cash.Value = 1milion     
	end      
end)      
...
print("Astralhook.dev")
print("Made by karlo#0002")
print("Ui made by rit")
print("Dev that helped alot Lucas")
print("Ty for using the script")

print("Astralhook.dev",Color3.new(0,1,0))
...
ChatScript.createNewMessage(" Astralhook.dev","Version: 0.0.1",MainUIColor,Color3.new(1,1,1),0.01,nil)	
